...

{
	...
	echo "mv -f \"${REMOTE_DIR}/.area-registry-countries.json.${UPLOAD_ID}.tmp\" \"${REMOTE_DIR}/area-registry-countries.json\""
	echo "mv -f \"${REMOTE_DIR}/.build-metadata.json.${UPLOAD_ID}.tmp\" \"${REMOTE_DIR}/build-metadata.json\""
	echo "mv -f \"${REMOTE_DIR}/.world-admin.pmtiles.${UPLOAD_ID}.tmp\" \"${REMOTE_DIR}/world-admin.pmtiles\""
	echo "bye"
} > "$LFTP_CMDS"
