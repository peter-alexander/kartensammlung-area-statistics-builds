...
needle = '"id":"urban-population.percent"'
matches = [index for index, line in enumerate(lines) if needle in line]
if len(matches) != 1:
	raise SystemExit(f"Expected exactly one WDI urbanization row, found {len(matches)}")
del lines[matches[0]]
...
heading = "### WDI versus UN World Urbanization Prospects (WUP 2025)"
if heading in policy:
	raise SystemExit("WUP audit section already present")
section = """### WDI versus UN World Urbanization Prospects (WUP 2025)
